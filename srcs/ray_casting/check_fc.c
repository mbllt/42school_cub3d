...
static int	intersct_plan_fc(t_vars *cub, t_vector ray, int i, t_dot_intersct *intersct)
{
	if (ray.z == 0)
	{
		return (0);
...
t_dot_intersct	check_wall_f(t_vars *cub, t_vector ray, t_dot_intersct *intersct)
{
	...
	{
		return ((t_dot_intersct){(*intersct).dot, (*intersct).t_distance, 4, ray});
	}
...
t_dot_intersct	check_wall_c(t_vars *cub, t_vector ray, t_dot_intersct *intersct)
{
	...
	{
		return ((t_dot_intersct){(*intersct).dot, (*intersct).t_distance, 5, ray});
	}
	return ((t_dot_intersct){(t_vector){0, 0, 0}, -1, -1, ray});
}